...
function buttonTwoClicked() {
    clear.innerHTML = " "
    title.innerText = "10 most spoken languages in world"
    ...
    const flatArray = languagesArray.flat()
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const countriesName = document.createElement("p")
        countriesName.innerText = tenValues[0]
        countriesName.style.width = "20%"
        ...
        const percentage = (tenValues[1] / sortedArray.length) * 100
        percentagediv.style.width = `${percentage}%`
        percentagediv.innerText = `${percentage.toFixed(2)}%`
        ...
        populationdiv.innerText = tenValues[1]
        onediv.appendChild(populationdiv)
